function [spikes] = meta_spike_extract(data,params,blanking_option)
spikes=[];
% spikes = MEA_spikes;
...
switch blanking_option
    case 1 % blanking times for 5x20Hz stim - cut off whole stim period
           spikes.blanking_times=1:6100; 
    case 2 % blanking times for 5x20Hz stim - cut off peri-stim only
           spikes.blanking_times=[1:2000,1980:2100,... 
                                  2980:3100,... 
                                  3980:4100,...
                                  4980:5100,...
                                  5980:6100]; 
    case 3 % blanking times for 5x50Hz stim - cut off whole stim period
           spikes.blanking_times=1:4100;
    case 4 % blanking times for 5x50Hz stim - cut off peri-stim only
...
spikes.padding_time=[10, 20]; %[10, 20]
% totallength=0;
% spikes_sparse=[];
...
%%
%  figure
%  for plot_id=1:spikes.clusters.no_spikes.clusters
%      hold on
%     spikes.clusters.histo(:,plot_id)=histc(spikes.clusters.spiketimes(spikes.clusters.assignments==spikes.clusters.cluster_ids(plot_id),:),1:200:20000);
