...
for file_idx=1:numel(meta.archive.WT_filenames)
     file_idx
    L4=meta.exported.WT_L4_MUA{file_idx};
    L23=meta.exported.WT_L23_MUA{file_idx};
    x_corr = pairwise_xcorr_MUA(L4,L23);
...
clear active_file file_idx no_files channel_index chosen_channel sweep_id temp_MUA temp_PDF this_sweep
clear functions
clear hidden
%% Indexes exported data directory and iteratively loads files for measurements
% KO loop
for file_idx=1:numel(meta.archive.KO_filenames)
    file_idx
    L4=meta.exported.KO_L4_MUA{file_idx};
    L23=meta.exported.KO_L23_MUA{file_idx};
...
figure 
for file_idx=1:numel(meta.archive.WT_filenames)
subplot(4,4,file_idx); hold on
imagesc(meta.params.T/20000-0.1,...
    meta.params.L/meta.params.bin_size_in_ms,...
    meta.WT_exported.x_corr.mean_c_grand{file_idx})
...
imagesc(meta.params.T/20000-0.1,...
    meta.params.L/meta.params.bin_size_in_ms,...
    meta.KO_exported.x_corr.mean_c_grand{file_idx})
