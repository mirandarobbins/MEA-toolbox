function spikes=meta_MUA64plot(data,params,spikes)
...
spikes.PDF.kernel_sigma =0.005;% .01; % finer detail =0.005
...
for channel_id=1:size(spikes.PDF.data_aligned,2)
    ...
    for trial_id=1:numel(data.sweep_sort.successful_sweeps); % to include successful trials only
...
%         subaxis(8,8,plot_id, 'Spacing', 0.01, 'Padding', 0, 'Margin', 0.01);
%     hold on
%     plot(data.tb,squeeze(data.filtered_spikes(:,plot_id,trial_to_plot)),'k')
%         plot(spikes.PDF.mean_PDF(plot_id,:),'r','LineWidth',1.5)
%
% %         text(800,0.07,strcat('Chan. ',num2str(channels_to_plot(plot_id))),'FontWeight','bold')
%
%     axis([0 1000 -0.2 0.2])
