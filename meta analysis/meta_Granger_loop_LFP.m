...
%     L23=meta.archive.WT_channels_chosen(file_idx,2);
    ...
    for idx=data.sweep_sort.successful_sweeps
        ...
        granger.X       =   downsample(data.filtered_lfp(4000:20000,:,params.selected_rep),20)'; % One rep
        % granger.X       =   downsample(CSD.csd_array(4000:20000,:,params.selected_rep),20)'; % One rep
        
        % granger.X       =   downsample(reshape(data.filtered_spikes(4000:20000,:,:),160010,64,1),10)'; % All reps
        temp_file=data.this_file; temp_file(numel(temp_file)-3:numel(temp_file))=[];
        granger.sfile   =[temp_file,'_sweep_',num2str(params.selected_rep),'.net']; clear temp_file
        ...
        % %     granger.X(chan_id,:)= reshape(spikes.PDF.PDF_trimmed{chan_id},1,numel(spikes.PDF_trimmed{chan_id})); % all trials
        %     granger.X(chan_id,:)= spikes.PDF.PDF_trimmed{chan_id}(idx,:); % selected rep
        % end; granger.X(isnan(granger.X))=0;
        ...
        % figure(3); clf reset;
        % figure; cca_plotcausality(granger.GC2,[],50); camroll(270); set(gca,'pos',[0.1 0.1 0.8 0.8]);
        
        figure; cca_plotcausality64(granger.GC2,[],50); camroll(270); set(gca,'pos',[0.1 0.1 0.8 0.8]);
        ...
        granger.GC_byTrial(:,:,idx)= granger.GC2;
    end
    granger.GC_byTrial_mean=nanmean(granger.GC_byTrial,3);
    figure; cca_plotcausality64(granger.GC_byTrial_mean,[],50); camroll(270); set(gca,'pos',[0.1 0.1 0.8 0.8]);
    meta.WT_exported.granger{file_idx}=granger;
...
%     L23=meta.archive.KO_channels_chosen(file_idx,2);
    ...
    for idx=data.sweep_sort.successful_sweeps
        ...
        % check that residuals are white
        granger.dKOhresh = 0.05/granger.nvar;    % critical threshold, Bonferroni corrected
        granger.waut = zeros(1,granger.nvar);
        for ii=1:granger.nvar,
            if granger.ret.waut<granger.dKOhresh,
                granger.waut(ii)=1;
            end
        end; clear ii
        granger.inx = find(granger.waut==1);
    ...
    granger.GC_byTrial_mean=nanmean(granger.GC_byTrial,3);
    figure; cca_plotcausality64(granger.GC_byTrial_mean,[],50); camroll(270); set(gca,'pos',[0.1 0.1 0.8 0.8]);
    meta.KO_exported.granger{file_idx}=granger;
    
end

%% extract stats on signal flow
for file_idx=1:numel(meta.archive.WT_filenames)
    L4=meta.archive.WT_channels_chosen(file_idx,1);
    L23=meta.archive.WT_channels_chosen(file_idx,2);
    meta.stats.granger.L4L23_flow_WT(file_idx)=meta.WT_exported.granger{file_idx}.GC_byTrial_mean(L4,L23);
    meta.stats.granger.L23L4_flow_WT(file_idx)=meta.WT_exported.granger{file_idx}.GC_byTrial_mean(L23,L4);
end

for file_idx=1:numel(meta.archive.KO_filenames)
    L4=meta.archive.KO_channels_chosen(file_idx,1);
    L23=meta.archive.KO_channels_chosen(file_idx,2);
    meta.stats.granger.L4L23_flow_KO(file_idx)=meta.KO_exported.granger{file_idx}.GC_byTrial_mean(L4,L23);
    meta.stats.granger.L23L4_flow_KO(file_idx)=meta.KO_exported.granger{file_idx}.GC_byTrial_mean(L23,L4);
end
[meta.stats.granger.L4L23_flow_test.h meta.stats.granger.L4L23_flow_test.p]=ttest2(meta.stats.granger.L4L23_flow_WT,meta.stats.granger.L4L23_flow_KO);
[meta.stats.granger.L23L4_flow_test.h meta.stats.granger.L23L4_flow_test.p]=ttest2(meta.stats.granger.L23L4_flow_WT,meta.stats.granger.L23L4_flow_KO);