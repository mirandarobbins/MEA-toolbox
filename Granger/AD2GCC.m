...
granger.NLAGS   =   10;         % if -1, best model order is assessed automatically
granger.Fs      =   500;         % params.Fs;   % sampling frequency  (for spectral analysis only)
granger.freqs   =   1:100;      % frequency range to analyze (spectral analysis only)
params.selected_rep=4;
granger.X       =   downsample(data.filtered_lfp(4000:20000,:,params.selected_rep),10)';
granger.N       =   size(granger.X,2);       % number of data points

granger.sfile   =   [data.this_file,'_sweep_',num2str(params.selected_rep),'.net'];
granger.nvar    =   size(granger.X,1);
...
figure(2); clf reset;
% subplot(232);
set(gca,'FontSize',granger.FSIZE-2);
imagesc(granger.GC2);
axis('square');
set(gca,'Box','off');
...
figure(3); clf reset;
subplot(233);cca_plotcausality64(granger.GC2,[],5); camroll(270); set(gca,'pos',[0 0 1 1])
set(gca,'FontSize',granger.FSIZE);
