...
function [data] = mload1ch( inFilename, DESIRED_CH, DESIRED_TRACES, MEM_LIMIT )

%MLOAD1ch	Loads data from a medac data file (Spontaneous Data). 
%	[data] = medload( inFilename, DESIRED_CH, DESIRED_TRACES [1], MEM_LIMIT [20 MB] ) 
%	loads the desired trace (one is default) of the disired channel from the data file defined 
...
%fd = fopen( inFilenameData );  % open header file                  <===Modified by T.H. 2004/12/21
fd = fopen( inFilenameData , 'r', 'l');  % open header file         <===Modified by T.H. 2004/12/21
...
data = zeros( nBlocks * oneShotSize, 2 );  % first column for the time stamp
%scaleFactor = amplitude * gain * 10 / 2048 / 50;                           %<=== Modified by T.H. 2004/12/21
scaleFactor = amplitude / 1024 * 2.5 / gain;                                %<=== Modified by T.H. 2004/12/21
...
for traceNum = DESIRED_TRACES
   ...
   for block = 1:nBlocks
     ...
     data(1 + (block - 1) * oneShotSize:oneShotSize + (block - 1) * oneShotSize, 2) = ...
         data(1 + (block - 1) * oneShotSize:oneShotSize + (block - 1) * oneShotSize, 2) + dataBlock(:, DESIRED_CH);
