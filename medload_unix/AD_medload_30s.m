function [params, data] = AD_medload_30s(fname)
% data=MEA_data;
% params=MEA_params;
if nargin <1
%     fname='20120426012';
...
params.degree = 1; 
params.window_retained=1:600000;% keep 1st 1000ms only
...
params.flags.prune_failures=0;
params.flags.plot_online=1;
params.flags.mask_channels=0;
params.flags.downsample=1;
end
...
for sweep_id=1:params.last_sweep
    ...
    temp=mload(data.this_file,sweep_id,1000); % open,load to {sweep}(timebase,channel)
...
params.rec_duration=params.no_points./params.Fs;
...
if params.flags.downsample==1
    data.tb=downsample(data.tb,20);
    data.raw_data=downsample(data.raw_data,20);
    params.Fs=1000;
    params.Nyquist=params.Fs/2;
    params.no_points=30000;
    params.window_retained=1:params.no_points;
end
% denoise 50Hz with Chronux
params.denoise.tapers=[30 5]; %[3 5];
params.denoise.fpass=[0 100];
params.denoise.Fs=params.Fs;
...
%     data.trimmed=data.raw_data(:,:,data.sweep_sort.successful_sweeps);
    ...
    data.sweep_sort.successful_sweeps=1;
...
params.bp.wn_high=[100/params.Nyquist 1000/params.Nyquist];
...
if params.last_sweep==1
   data.mean_channels=data.filtered_lfp;
   data.std_channels=zeros(size(data.mean_channels));
else
    for channel_id=1:params.no_channels
...
for channel_id=1:params.no_channels
       % tag LFP minima 
       [data.max_amp(channel_id) data.latency(channel_id)]=...
                min(data.filtered_lfp(params.search_win_samples(1):params.search_win_samples(2),channel_id,params.selected_rep)); 
    ...
    if gt(mean(data.mean_channels(params.monosynaptic_win_samples(1):params.monosynaptic_win_samples(2),channel_id)),...
       ...
       [data.max_amp_mono(channel_id) data.latency_mono(channel_id)]=...
                max(data.mean_channels(params.monosynaptic_win_samples(1):params.monosynaptic_win_samples(2),channel_id)); % plot selected rep
    else
       [data.max_amp_mono(channel_id) data.latency_mono(channel_id)]=...
                min(data.mean_channels(params.monosynaptic_win_samples(1):params.monosynaptic_win_samples(2),channel_id)); % plot selected rep
    end
end
...
if params.flags.plot_online==1
    time_limit=params.rec_duration*1000;
