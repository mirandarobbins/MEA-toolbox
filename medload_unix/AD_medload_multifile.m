function [params, data] = AD_medload_multifile(parent,first,last,params)
...
if nargin<3
% construct filename
file.parent=input('Input file parent string : ','s');
file.first = input('Select first file to import : ','s');
file.last = input('Select last file to import : ','s');
else
...
params.search_win=([200, 800]);%for 5x50Hz
...
%     this_file=params.files.MEA.FileListShort{sweep_id};
%     temp=mload(this_file,1,100); % open,load to {sweep}(timebase,channel)
