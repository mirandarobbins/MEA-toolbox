...
params.search_win=([220,420]);%for 5x50Hz
% params.search_win=([320, 800]);%for 5x20Hz
...
for file_id=1:params.last_sweep
    ...
    for sweep_id=1:100
        try
            temp{sweep_id}=mload(this_file,sweep_id,100); % open,load to {sweep}(timebase,channel)
    ...
    sweep_id=sweep_id-1;
    data.tb=temp{1}(:,1); %remove timebase column
