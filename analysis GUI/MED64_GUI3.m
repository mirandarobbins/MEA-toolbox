function varargout = MED64_GUI3(varargin)
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @MED64_GUI3_OpeningFcn, ...
                   'gui_OutputFcn',  @MED64_GUI3_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
%%%%%%%%%%%%%%%% Init CALLBACKS
function MED64_GUI3_OpeningFcn(hObject, eventdata, handles, varargin) 

handles.output = hObject; % Choose default command line output for MED64_GUI
guidata(hObject, handles); % Update handles structure
function varargout = MED64_GUI3_OutputFcn(hObject, eventdata, handles)
...
handles.params = evalin('base','params');
t2=toc;     
disp(strcat('Completed loading in........', num2str(t2),'s'))
displaystats(hObject, eventdata, handles)
LFP_all_overlaid(hObject, eventdata, handles) % LFP all sweeps overlaid
...
handles.data = evalin('base','data');
handles.params = evalin('base','params');
AD_xsgload_multifile(handles.data,handles.params);
t2=toc;     
disp(strcat('Completed loading in........', num2str(t2),'s'))
% displaystats(hObject, eventdata, handles)
...
switch popup_sel_index
    ...
    case 18    
        MUA_LFP_overlaid(hObject, eventdata, handles)
...
%         cla; 
        ...
        plot(handles.data.tb,squeeze(handles.data.filtered_lfp(:,channel_id,this_rep)),'k')
        else
        plot(handles.data.tb,squeeze(handles.data.filtered_lfp(:,channel_id,this_rep)),'color','k');%[0.3 0.3 0.3])
        end
        axis([200 400 -0.5 0.25])
        set(gca,'xtick',[])
        set(gca,'ytick',[])
%         text(25,0.1,num2str(channel_id),'FontWeight','bold')
end
...
for plot_id=1:handles.params.no_channels
        ...
        cla; hold on ;box off; axis off
        plot(handles.data.tb,handles.data.mean_channels(:,plot_id),'k','LineWidth',1)
...
%         ciplot((handles.data.mean_channels(:,plot_id)-handles.data.std_channels(:,plot_id)),...
%                (handles.data.mean_channels(:,plot_id)+handles.data.std_channels(:,plot_id)),handles.data.tb,'b');
        plot(handles.data.tb,handles.data.mean_channels(:,plot_id),'k','LineWidth',2)
        area(handles.data.tb,handles.data.mean_channels(:,plot_id))
...
function PlotChosenChannel(hObject, eventdata, handles) % plot each rep on strongest channel
    ...
    x_shift=0; 
    y_shift=10;
    temp1  =  handles.spikes.PDF.PDF_trimmed{ChosenChannel}(:,200:400)'; temp1(isnan(temp1))=0;
    temp2  =  (squeeze(handles.data.filtered_lfp   (4000:8000,ChosenChannel,:)));
    temp3  =  (squeeze(handles.data.filtered_spikes(4000:8000,ChosenChannel,:)));
   
    temp2  =  downsample(temp2,1);
    temp3  =  downsample(temp3,1);
    temp1  =  staggerplot(zscore(temp1,[],1),x_shift,y_shift);
    temp2  =  staggerplot(2*zscore(temp2),x_shift,y_shift);
    temp3  =  staggerplot(0.5*zscore(temp3),x_shift,y_shift);
    
    hold on; axis off; box off
    plot(temp2,'color','r','LineWidth',1.1)
    plot(3+temp3,'color',[0.3 0.3 0.3],'LineWidth',1)
%     area(2+temp1,'FaceColor',[0.6 0.6 0.6],'EdgeColor',[0.3 0.3 0.3],'LineWidth',1.1)
%     plot(2+temp1,'color',[0.3 0.3 0.3],'LineWidth',1)
...
function MUA_LFP_overlaid(hObject, eventdata, handles) % plots grid of MUA and LFP overlaid
figure('Name',strcat('Mean LFP and MUA ovelaid'),'NumberTitle','off')
for plot_id=1:handles.params.no_channels
        subaxis(8,8,plot_id, 'Spacing', 0.01, 'Padding', 0, 'Margin', 0.01);
        cla; hold on ;box off; axis off
        plot(handles.data.tb,...
            sgolayfilt(squeeze(handles.data.filtered_spikes(:,plot_id,handles.data.sweep_sort.successful_sweeps)),0,15)...
                            ,'color',[0.6 0.6 0.6])
        plot(handles.data.tb,handles.data.mean_channels(:,plot_id),'k','LineWidth',1) % plot LFP
