...
function [data] = medload( inFilename, DESIRED_TRACES, MEM_LIMIT )

%MEDLOAD	Loads data from a medac data file. 
...
%--- Check if it is converted data or not ---
frewind( fh )  % start search at the begining of the file
oneLine = fgetl( fh );

while( ~strncmp( CONVERT_LABEL, oneLine, length( CONVERT_LABEL) ) & ~feof( fh ) )
   oneLine = fgetl( fh ); 
end
...
if (convertFlag ~= 1)
    ...
    oneShotSize = fragmentSize;
end

%--- Extract 'One Record Size/CH'
    frewind( fh )  % start search at the begining of the file
    oneLine = fgetl( fh );

    while( ~strncmp( ONE_RECORD_SIZE_LABEL, oneLine, length( ONE_RECORD_SIZE_LABEL ) ) & ~feof( fh ) )
        oneLine = fgetl( fh ); 
    end

    if( feof( fh ) )
        oneRecordSize = fragmentSize;
    else
        oneRecordSize = readLabeledValue( fh, ONE_RECORD_SIZE_LABEL );
    end
