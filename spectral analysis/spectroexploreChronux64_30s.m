function [data params spectro fourier] = spectroexploreChronux64_30s(data,params)
...
%         plot(data.tb,sgolayfilt(squeeze(data.filtered_lfp(:,spectro.channeltoanalyse,data.sweep_sort.successful_sweeps)),0,15),'Color',[0.8 0.8 0.8]); 
        ...
        axis([min(min(spectro.T)) max(max(spectro.T)) -0.2 0.2])
        xlabel('time (ms)')    
