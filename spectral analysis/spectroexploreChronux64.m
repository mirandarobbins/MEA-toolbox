function [data params spectro fourier] = spectroexploreChronux64(data,params)
...
spectro.params.T         =   0.1; % duration to calculate tapers over (in s)  /0.1
spectro.params.tapers    =   [spectro.params.W     spectro.params.T    0];
spectro.params.tapers    =   [3 5]; % /off <<<<< NORMAL
% spectro.params.tapers    =   [5 9]; % /off
spectro.params.Fs        =   params.Fs        ;
spectro.params.pad       =   3;
spectro.params.err       =   [2 1];
spectro.params.trialave  =   0;
spectro.params.movingwin =   [spectro.params.T spectro.params.T/20]; %<<<<< NORMAL
...
if plotYN==1
    ...
    subplot(311); hold on
        ...
        plot(data.tb,mean(squeeze(data.filtered_lfp(:,spectro.channeltoanalyse,data.sweep_sort.successful_sweeps)),2),'-k')
    ...
    %  set(gca,'YScale','log')
        ...
        ax2b = axes('Position',get(ax2a,'Position'),...
                   ...
                   'YTick',[],...
                   'xlim',[0, 1],...
                   'ylim',[0, spectro.params.fpass(2)]);       
        rectangle('Position',[params.search_win(1)/1000,...
                             min(spectro.F),...
                             (params.search_win(2)-params.search_win(1))/1000,...
                             max(spectro.F)-min(spectro.F)],...
                             'LineWidth',2,'LineStyle','-','EdgeColor',[0 0 1],'Parent',ax2b)
%     subplot(313);hold on;
%         channel_to_plot=spectro.channeltoanalyse;
%         % for sweep_id=data.sweep_sort.successful_sweeps       
%         %     plot(fourier.f,fourier.psd(:,channel_to_plot,sweep_id),':b')
%         % end
%         % semilogx(fourier.f,fourier.psd_mean_success(:,channel_to_plot),'b','LineWidth',1.5)
%         % ciplot(fourier.psd_mean_success(:,channel_to_plot)-fourier.psd_SEM_success(:,channel_to_plot),...
%         %        fourier.psd_mean_success(:,channel_to_plot)+fourier.psd_SEM_success(:,channel_to_plot),...
%         %        fourier.f,'b')
%         % if ~isempty(data.sweep_sort.failures)
%         %     for sweep_id=data.sweep_sort.failures       
%         %         plot(fourier.f,fourier.psd(:,channel_to_plot,sweep_id),':r')
%         %     end   
%         %     semilogx(fourier.f,fourier.psd_mean_failure(:,channel_to_plot),'r','LineWidth',1.5)
%         %     ciplot(fourier.psd_mean_failure(:,channel_to_plot)-fourier.psd_SEM_failure(:,channel_to_plot),...
%         %            fourier.psd_mean_failure(:,channel_to_plot)+fourier.psd_SEM_failure(:,channel_to_plot),...
%         %            fourier.f,'r')
%         % end
%         % plot(spectro.F,spectro.average_window_power)%,':b')
%         for trial_id=data.sweep_sort.successful_sweeps
%                 if data.sweep_sort.window_std(channel_to_plot,trial_id)>params.detection_threshold
...
end; fourier=rmfield(fourier,'g'); clear trial_id
...
if plotYN==1        
    figure; hold on
    plot(fourier.f,fourier.psd_mean_success(:,channel_to_plot),'k')
    ciplot(fourier.psd_mean_success(:,channel_to_plot) - fourier.psd_SEM_success(:,channel_to_plot),...
           fourier.psd_mean_success(:,channel_to_plot) + fourier.psd_SEM_success(:,channel_to_plot),...
           fourier.f,'k')
    xlabel('Frequency (Hz)')
